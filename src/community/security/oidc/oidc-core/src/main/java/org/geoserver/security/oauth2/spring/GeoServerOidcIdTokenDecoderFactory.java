...
/**
 * GeoServer factory for OIDC token decoding allows to replace the default Spring {@link
 * OidcIdTokenDecoderFactory}. Required to support reconfiguration through the GS admin UI. The
 * {@link #delegate} has to be replaced to use empty stale caches.
 *
 * @author awaterme
 */
    ...
    public JwtDecoder createDecoder(ClientRegistration pContext) {
        if (delegate == null) {
            throw new IllegalStateException(
                    "Decoder creation failed. Required configuration is missing.");
        }
