...
import static org.geoserver.security.oauth2.common.GeoServerOAuth2UserServices.oauth2UserService;
import static org.geoserver.security.oauth2.common.GeoServerOAuth2UserServices.oidcUserService;
...
public class GeoServerOAuth2LoginAuthenticationFilterBuilder
        implements GeoServerOAuth2ClientRegistrationId {
    ...
    public GeoServerOAuth2LoginAuthenticationFilter build() {
        ...
        GeoServerOAuth2LoginAuthenticationFilter filter =
                new GeoServerOAuth2LoginAuthenticationFilter();
        ...
        ClientRegistration lReg =
                ClientRegistration
                        ...
                        .providerConfigurationMetadata(
                                singletonMap(
                                        "end_session_endpoint", configuration.getOidcLogoutUri()))
    ...
    public LogoutSuccessHandler getLogoutSuccessHandler() {
        if (logoutSuccessHandler == null) {
            ...
            lLogoutSuccessHandler.setPostLogoutRedirectUri(
                    configuration.getPostLogoutRedirectUri());
    ...
    public OAuth2UserService<OAuth2UserRequest, OAuth2User> getOauth2UserService() {
        if (oauth2UserService == null) {
            oauth2UserService =
                    oauth2UserService(
                            getRoleResolverContext(), getRequestSupplier(), configuration);
        }
    ...
    public void setOauth2UserService(
            OAuth2UserService<OAuth2UserRequest, OAuth2User> pOAuth2UserService) {
    ...
    public OAuth2UserService<OidcUserRequest, OidcUser> getOidcUserService() {
        if (oidcUserService == null) {
            oidcUserService =
                    oidcUserService(getRoleResolverContext(), getRequestSupplier(), configuration);
        }
    ...
    public void setRoleResolverContext(
            GeoServerRoleResolvers.ResolverContext pRoleResolverContext) {
    ...
    public void setClientRegistrationRepository(
            InMemoryClientRegistrationRepository pClientRegistrationRepository) {
    ...
    public OAuth2AuthorizedClientService getAuthorizedClientService() {
        if (authorizedClientService == null) {
            authorizedClientService =
                    createAuthorizedClientService(getClientRegistrationRepository());
        }
    ...
    public OAuth2AuthorizedClientRepository getAuthorizedClientRepository() {
        if (authorizedClientRepository == null) {
            authorizedClientRepository =
                    createAuthorizedClientRepository(getAuthorizedClientService());
        }
    ...
    public void setAuthorizedClientRepository(
            OAuth2AuthorizedClientRepository pAuthorizedClientRepository) {
    ...
    public Filter getRedirectToProviderFilter() {
        if (redirectToProviderFilter == null) {
            ...
            RequestMatcher lMatcher =
                    r -> {
                        Authentication lAuth =
                                SecurityContextHolder.getContext().getAuthentication();
                        if (lAuth == null) {
                            return true;
                        }
                        boolean lFullyAuthenticated =
                                !trust.isAnonymous(lAuth) && !trust.isRememberMe(lAuth);
                        return !lFullyAuthenticated;
                    };
            redirectToProviderFilter =
                    new RequestMatcherRedirectFilter(
