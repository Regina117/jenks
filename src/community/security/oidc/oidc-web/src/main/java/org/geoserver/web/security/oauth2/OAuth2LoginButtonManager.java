...
import org.geoserver.security.oauth2.OAuth2LoginButtonEnablementEvent;
import org.geoserver.web.LoginFormInfo;
import org.springframework.context.event.EventListener;

/**
 * Enables login buttons for OAuth2 dynamically, depending on the respective providers enablement
 * state. Required since a single filter instance supports multiple providers, so the regular
 * enablement of login buttons based on the presence of a filter is not sufficient.
 */
public class OAuth2LoginButtonManager {

    private List<LoginFormInfo> loginFormInfos = new ArrayList<LoginFormInfo>();
