...
/**
 * Provides {@link RoleResolver}s to obtain {@link GeoServerRole}s for a principal name during
 * authentication. Useful for filters in pre-authentication scenarios.
 *
 * <p>To resolve roles provide a {@link ResolverContext} with the contextual information regarding
 * the sources to inspect and a {@link ResolverParam} with the current principal name and {@link
 * HttpServletRequest}.
 *
 * <p>Typically the {@link #PRE_AUTH_ROLE_SOURCE_RESOLVER} should be used which covers the other
 * resolvers.
 */
public class GeoServerRoleResolvers {
    ...
    /**
     * Provides the {@link #principal} name and {@link #request} together with the {@link #context}
     * which provides access to further information required for the conversion.
     */
    public static class ResolverParam {
         ...
         * @param pContext
         */
        public ResolverParam(
                String pPrincipal, HttpServletRequest pRequest, ResolverContext pContext) {
            super();
    ...
    /**
     * Provides access to the {@link RoleSource}. It determines which source shall be considered.
     * Also provides the name to be used per source.
     */
     ...
     */
    public static final RoleResolver ROLE_SERVICE_RESOLVER =
            p -> {
                boolean useActiveService =
                        p.getRoleServiceName() == null
                                || p.getRoleServiceName().trim().length() == 0;
                ...
                try {
                    service =
                            useActiveService
                                    ...
                                    : p.getSecurityManager()
                                            .loadRoleService(p.getRoleServiceName());
     ...
     */
    public static final RoleResolver USER_GROUP_SERVICE_RESOLVER =
            p -> {
                ...
                try {
                    service =
                            p.getSecurityManager()
                                    .loadUserGroupService(p.getUserGroupServiceName());
                } catch (IOException e) {
                    throw new GeoServerRuntimException(
                            "Failed to load roles for user '"
                                    + p.principal
                ...
                try {
                    ...
                    LOGGER.log(
                            Level.WARNING,
                            "User " + p.principal + " not found in " + p.getUserGroupServiceName());
                }
    ...
    public static final RoleResolver HTTP_HEADER_RESOLVER =
            p -> {
                ...
                if (rolesString == null || rolesString.trim().length() == 0) {
                    LOGGER.log(
                            Level.WARNING,
                            "No roles in header attribute: " + p.getRolesHeaderAttribute());
                    return roles;
                }
    ...
    /**
     * Resolves {@link GeoServerRole}s when the {@link RoleSource} is a {@link
     * PreAuthenticatedUserNameRoleSource}.
     */
    public static final RoleResolver PRE_AUTH_ROLE_SOURCE_RESOLVER =
            p -> {
